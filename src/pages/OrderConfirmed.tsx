    ...
    <div className="container-x py-16 lg:py-24">
      <div className="mx-auto max-w-2xl text-center">
        <motion.div
          ...
          className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-accent/12"
        >
          <motion.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
        ...
        {order && (
          <motion.p
            ...
            className="mt-5 inline-flex items-center gap-2 rounded-full bg-card px-4 py-2 text-[13px] shadow-soft"
          >
            <span className="text-text-muted">Order reference</span>
            <strong className="font-mono tracking-wider text-accent">{order.ref}</strong>
          </motion.p>
        )}
      </div>
      ...
      {order && (
        <motion.div
          ...
          className="mx-auto mt-12 max-w-2xl overflow-hidden rounded-2xl bg-card shadow-soft"
        >
          <div className="grid gap-px bg-bg-2/8 sm:grid-cols-3">
            {[
              ...
              <div key={l} className="bg-card p-5">
                ...
                <p className="mt-2.5 text-[10.5px] uppercase tracking-[0.14em] text-text-dim">{l}</p>
          ...
          <div className="p-6 sm:p-8">
              ...
              {order.items.map((it) => (
                <li key={it.name} className="flex items-center justify-between gap-4 py-3 text-[13.5px]">
                  <span className="min-w-0">
                    ...
                    <span className="text-[12px] text-text-dim">Qty {it.qty}</span>
                  </span>
            ...
            <dl className="mt-5 space-y-2 border-t border-white/10 pt-5 text-[13.5px]">
              ...
              <div className="flex justify-between">
                ...
                <dd className="tabular-nums">
                  {order.delivery === 0 ? <span className="text-accent">Free</span> : UGX(order.delivery)}
                </dd>
              </div>
            ...
            <div className="mt-6 rounded-xl bg-bg p-5">
              <p className="text-[10.5px] uppercase tracking-[0.14em] text-text-dim">Delivering to</p>
              <address className="mt-1.5 text-[13.5px] not-italic leading-relaxed text-text-muted">
                <strong className="text-text">{order.delivery_details.fullName}</strong><br />
